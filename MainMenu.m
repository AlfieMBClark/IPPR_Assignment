function MainMenu()
    ...
    function openRubberNitrile(~, ~)
        try
            addpath(rootDir);  % Parent directory
            rubberPath = fullfile(rootDir, 'RubberNitrileGloves');
            addpath(rubberPath);
            RubberNitrile_GUI();
            % Del menu
            delete(fig);
    ...
    function openCloth(~, ~)
        try
            addpath(rootDir);
            clothPath = fullfile(rootDir, 'Cloth Gloves');
            addpath(clothPath);
            GDDS();
            delete(fig);
        catch ME
            uialert(fig, sprintf('Error launching Cloth GUI:\n%s', ME.message), 'Error');
        end
    end
